...
Future<void> bgUpdateCheck(
  ...
  if (canInstall &&
    ...
    for (final id in discovered) {
      ...
      // Re-read rather than force-unwrap: `discovered` is a snapshot, and the
      // awaits in this loop give a concurrent loadApps() the chance to drop an
      // app that was uninstalled outside Obtainium mid-check.
      final app = appsProvider.apps[id]?.app;
      if (app == null) continue;
      if (await appsProvider.canInstallSilentlyInBackground(app)) {
  ...
  if (params['toCheck'] == null) {
    // Installing replaces a running app and kills it, so unless the user has
    // opted out, hold the whole batch until the phone is locked. Most checks
    // have nothing to install, so only ask then.
    if (silentlyInstallable.isNotEmpty &&
        !await appsProvider.canInstallForScreenState()) {
      AppLogger.info(
        'BG install task: Skipping ${silentlyInstallable.length} app(s): the '
        'device is not locked. They will be retried on the next check.',
      );
      silentlyInstallable.clear();
    }
