...
/// Whether an archive entry name stays inside the directory it is extracted
/// into. Rejects absolute paths and any '..' segment, both of which would let a
/// crafted archive write over files elsewhere on the device.
bool _isSafeArchiveEntryName(String name) {
...
extension AppsProviderInstall on AppsProvider {
  ...
  Future<void> _installQueuedApp(
    ...
    // The batch was checked before downloading, but a download takes long
    // enough for the user to have picked the phone up since. This is the last
    // point at which the update can still be left for next time.
    if (context == null &&
        res.willBeSilent &&
  ...
  Future<void> _downloadAssetFile(
    ...
    for (var i = 0; i < urls.length; i++) {
      ...
      try {
        ...
        final File savedFile = await downloadFileWithRetry(
          fileName,
          true,
        ...
        unawaited(
          notificationsProvider.notify(
            // Name the file that actually landed: a destination this app may
            // not overwrite makes the download fall back to a `name (n).ext`
            // variant.
