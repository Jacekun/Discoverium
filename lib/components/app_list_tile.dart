...
class AppListTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasUpdate = appHasOfferableUpdate(
      _app,
...
class AppListBuilder {
  static List<AppInMemory> filter(
    ...
    return apps.where((app) {
      // "Up to date" is the absence of an update, not string equality: an
      // installed build that is ahead of the source's latest has nothing to
      // offer, and must not survive this filter just because the two strings
      // differ. Not-installed apps are handled by the next check instead.
      //
      // appHasOfferableUpdate composes both rules: the fork's ordering (which
      // alone handles pseudo-versions) and the hideDowngrades preference.
...
class _VersionLabel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasUpdate = appHasOfferableUpdate(app, settingsProvider);
  ...
  bool isVersionUpdate(App app) => appHasOfferableUpdate(app, settingsProvider);
