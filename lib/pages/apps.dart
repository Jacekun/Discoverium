  ...
  Future<void> showMassMarkDialog(
    ...
    try {
      ...
      for (final selected in selectedApps) {
        var current = appsProvider.apps[selected.id]?.app;
        ...
        if (current.installedVersion != null &&
            current.settings.getBool('trackOnly')) {
          current = current.copyWith(installedVersion: current.latestVersion);
        }
        changed.add(current);
      }
      unawaited(appsProvider.saveApps(changed));
  ...
  void shareAppURLs(Set<App> selectedApps) {
    ...
    unawaited(
      SharePlus.instance.share(
        ShareParams(text: urls, subject: 'Discoverium - ${tr('appsString')}'),
      ),
    );
  }
  ...
  void shareConfigLinks(Set<App> selectedApps) {
    ...
    unawaited(
      SharePlus.instance.share(
        ShareParams(
          text: buf.toString(),
          subject: 'Discoverium - ${tr('appsString')}',
        ),
      ),
    );
  }
...
class _BulkUpdateDialogState extends State<_BulkUpdateDialog> {
  ...
  Widget _appCheckRow(String id, ColorScheme cs) {
    ...
    final isUpdate = appHasOfferableUpdate(
      aim.app,
