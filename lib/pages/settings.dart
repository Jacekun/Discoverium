...
class _SettingsPageState extends State<SettingsPage> {
  /// Stable for the process lifetime, and each section is its own page, so
  /// hold it across States rather than re-probing on every navigation.
  ...
  Future<void> initAndroidSdk() async {
    if (_cachedAndroidSdkInt != null) {
      // Another page may have resolved it after this one was constructed.
      if (androidSdkInt != _cachedAndroidSdkInt && mounted) {
        setState(() => androidSdkInt = _cachedAndroidSdkInt);
      }
      return;
    }
    try {
      ...
      androidSdkInt = _cachedAndroidSdkInt;
      if (mounted) setState(() {});
    } catch (e) {
      if (!mounted) return;
  ...
  Future<bool> showColorPickerDialog(
    ...
    final Map<ColorSwatch<Object>, String> colorsNameMap =
        <ColorSwatch<Object>, String>{obtainiumSwatch: 'Discoverium'};
    return ColorPicker(
  ...
  Widget build(BuildContext context) {
    ...
    List<Widget> childrenFor(SettingsSection section) {
      switch (section) {
        ...
        case SettingsSection.updates:
          return _buildUpdatesSection(context, showBgSection, sdk);
        case SettingsSection.repoApps:
          return _buildRepoAppsSection(context, settingsProvider);
        case SettingsSection.sourceSpecific:
          return sourceSpecificForm == null ? const [] : [sourceSpecificForm];
        ...
        case SettingsSection.categories:
          return const [
            CardTile(padding: EdgeInsets.all(12), child: CategoryManager()),
          ];
      }
    }

    final loading = settingsProvider.prefs == null;
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: <Widget>[
          CustomAppBar(
            title: section == null ? tr('settings') : _sectionTitle(section),
          ),
          SliverPadding(
              ...
              child: loading
                  ...
                  : Column(
                      ...
                      children: section == null
                          ? _buildSectionMenu(
                              context,
                              hasSourceSpecific: sourceSpecificForm != null,
                            )
                          : shapeCardTiles(childrenFor(section)),
                    ),
            ),
          ),
          // The source-code and logs shortcuts belong to the menu, not to an
          // individual section.
          if (section == null) _buildFooter(context),
        ],
      ),
    );
  }
  ...
  String _sectionTitle(SettingsSection section) => switch (section) {
    ...
    SettingsSection.sourceSpecific => tr('sourceSpecific'),
    SettingsSection.appearance => tr('appearance'),
    SettingsSection.categories => tr('categories'),
  };
  ...
  IconData _sectionIcon(SettingsSection section) => switch (section) {
    ...
    SettingsSection.sourceSpecific => Icons.source_outlined,
    SettingsSection.appearance => Icons.palette_outlined,
    SettingsSection.categories => Icons.category_outlined,
  };

  List<Widget> _buildSectionMenu(
    BuildContext context, {
    required bool hasSourceSpecific,
  }) {
    final sections =
        SettingsSection.values
          ...
          // active locale rather than the enum's declaration order.
          ..sort(
            (a, b) => _sectionTitle(
              a,
  ...
  List<Widget> _buildRepoAppsSection(
    BuildContext context,
    SettingsProvider settingsProvider,
  ) {
    return [
      CardTile(
        ...
        child: DropdownButtonFormField<String>(
          ...
          initialValue: settingsProvider.discoveriumBranch,
          items: const [
          ...
          onChanged: (value) {
            ...
            // The cached repo belongs to the previous branch.
            DiscoveriumRepo.invalidate();
          },
        ),
      ),
      ToggleTile(
        label: tr('allowUnverifiedApps'),
        value: settingsProvider.allowUnverifiedApps,
        onChanged: (value) => settingsProvider.allowUnverifiedApps = value,
      ),
      ToggleTile(
        label: tr('allowCommercialApps'),
