...
/// Human-readable form of a SAF tree URI, e.g. a tree URI for the primary
/// volume's Download folder renders as `/Download`. Non-primary volumes keep
/// their raw volume prefix, since there is no name to substitute.
String prettyExportDirPath(Uri dir) =>
    dir.pathSegments.join('/').replaceFirst('tree/primary:', '/');
...
extension AppsProviderImportExport on AppsProvider {
  ...
  Future<String?> export({
    ...
    if (!pickOnly) {
      ...
      returnPath = prettyExportDirPath(exportDir);
    }
    return returnPath;
  }
  ...
  Future<MapEntry<List<App>, bool>> import(String appsJSON) async {
    ...
    // Saving records each app's installed version from the OS; a track-only
    // app keeps the release it was marked with.
  ...
  factory ExportSchema.fromJson(Map<String, dynamic> json) {
    ...
    if (schemaVersion > currentExportSchemaVersion) {
      throw FormatException(
        'Export was created by a newer version of Discoverium '
        '(schema v$schemaVersion, current is v$currentExportSchemaVersion). '
        'Please update Discoverium to import this file.',
      );
    }
    return ExportSchema(
