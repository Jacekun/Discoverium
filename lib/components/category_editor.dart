      ...
      for (final aim in appsProvider.apps.values) {
        if (aim.app.categories.contains(prev)) {
          aim.app = aim.app.copyWith(
            // Deduplicate: an app already in both `prev` and `name` would
            // otherwise end up listing `name` twice, which every other writer
            // of App.categories prevents by building the list from a Set.
